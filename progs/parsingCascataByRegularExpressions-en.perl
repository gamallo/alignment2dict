...
###ATENÇAO: SO PARA LINGUAS ROMANCES. NO CASO DO INGLES E PRECISO MUDAR A REGRA NOME-NOME!!!!!
...
###REGRA LEXICAL DAS PARTICULAS VERBAIS (juntamos a particula ao verbo)
##########Regra subs: V PCLE -> V
########## turn on -> turn&on
      (@temp) = ($listTags =~ /($VERB)($PCLE)/g);
       $listTags =~ s/($VERB)($PCLE)/$1/g ; 
       $Rel =  "@" ;
       VerbParticles($Rel, @temp);
#########Fim regra

###FIM REGRA LEXICAL DAS PARTICULAS
...
##########Regras NOM POS NOM para o ingles ('s) 
       (@temp) = ($listTags =~ /($NOM)($POS)($NOM)/g);
        $listTags =~ s/($NOM)($POS)($NOM)/$3/g ; 
        DepRelHead(@temp);
